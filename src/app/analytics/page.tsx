...
export default function Analytics() {
    ...
    async function fetchAnalytics() {
      ...
      try {
        ...
        if (fetchedLogs.length > 0) {
          ...
          const doseLogs = fetchedLogs.filter(
            (l) => l.dose || l.type === "dose",
          );
          if (doseLogs.length > 0) {
            setCurrentDose(doseLogs[doseLogs.length - 1].dose);
          }

          const weightLogs = fetchedLogs.filter(
            (l) => typeof l.weight === "number",
          );
  ...
  const generateSmoothPath = () => {
    ...
    const coords = chartLogs.map((p, i) => ({
      ...
      y:
        PAD_TOP +
        (1 - (p.weight - minW) / range) * (SVG_H - PAD_TOP - PAD_BOTTOM),
    }));
  ...
  const generateFillPath = () => {
    ...
    const endX =
      PAD_X +
      ((chartLogs.length - 1) / (chartLogs.length - 1)) * (SVG_W - PAD_X * 2);
  ...
  const maxW =
    chartLogs.length > 0 ? Math.max(...chartLogs.map((l) => l.weight)) : 100;
  const minW =
    chartLogs.length > 0 ? Math.min(...chartLogs.map((l) => l.weight)) : 80;
  const gridLines =
    chartLogs.length >= 2 ? [minW, minW + (maxW - minW) * 0.5, maxW] : [];
  const range = maxW - minW || 1;
  ...
  return (
    <ProtectedRoute>
      <main
        className="container"
        style={{ maxWidth: "1000px", paddingTop: "3rem", position: "relative" }}
      >
        ...
        >
          <Image
            src="/images/analytics-empty.png"
            alt=""
            fill
          ...
          <div
            style={{
              ...
              inset: 0,
              background:
                "linear-gradient(to bottom, rgba(11, 17, 32, 0.4) 0%, var(--bg-primary) 100%)",
            }}
          ></div>
        </div>
        <header
        ...
        >
          <div>
            ...
            <p className="page-subtitle" style={{ marginTop: "0.25rem" }}>
              Acompanhe peso, dose e ritmo da jornada.
            </p>
          </div>
          <div
        ...
        <div
          ref={captureRef}
          style={{
            padding: isCapturing ? "2rem" : "0",
            background: isCapturing ? "#0b1120" : "transparent",
            borderRadius: isCapturing ? "16px" : "0",
          }}
        >
          ...
          <div
            className={`glass-panel ${!isCapturing ? "anim-enter anim-delay-1" : ""}`}
            style={{
              padding: "2rem",
              marginBottom: "1.5rem",
              opacity: 1,
            ...
            {loading ? (
              ...
              >
                ...
                {gridLines.map((val) => {
                  const y =
                    PAD_TOP +
                    (1 - (val - minW) / range) * (SVG_H - PAD_TOP - PAD_BOTTOM);
                  return (
                ...
                {getPointCoords().map((pt, i) => (
                  <g key={`${pt.date}-${pt.weight}-${pt.x}`}>
                    ...
                    {(pt.isFirst ||
                      pt.isLast ||
                      i % Math.max(1, Math.floor(chartLogs.length / 4)) ===
                        0) && (
                      <text
                        x={pt.x}
                        y={pt.y - 16}
                    ...
                    <text
                      ...
                      fontFamily="DM Sans"
                    >
                      {new Date(pt.date + "T12:00:00").toLocaleDateString(
                        "pt-BR",
              ...
              >
                ...
                <p style={{ color: "var(--text-muted)", fontSize: "0.85rem" }}>
                  Registrado em{" "}
                  {new Date(chartLogs[0].date + "T12:00:00").toLocaleDateString(
                    "pt-BR",
                    { day: "2-digit", month: "long" },
                  )}
                </p>
                <p style={{ color: "var(--text-muted)", fontSize: "0.8rem" }}>
                  Adicione mais registros para visualizar a curva de evolução.
                </p>
              </div>
            ) : (
              <div
              ...
              >
                <Image
                  src="/images/analytics-empty.png"
                  alt="Ilustração de gráfico"
                  width={180}
                  height={135}
                  style={{ opacity: 0.5 }}
                />
                <p style={{ color: "var(--text-muted)" }}>
                  Registre seu primeiro dado para liberar os gráficos.
                </p>
              </div>
            )}
          </div>
          ...
          >
            <div
              className={`glass-panel ${!isCapturing ? "anim-enter anim-delay-1" : ""}`}
              style={{ padding: "1.5rem", opacity: 1, transform: "none" }}
            >
            ...
            <div
              className={`glass-panel ${!isCapturing ? "anim-enter anim-delay-2" : ""}`}
              style={{ padding: "1.5rem", opacity: 1, transform: "none" }}
            >
              <p className="stat-label">Média Semanal</p>
              <p
                className="stat-number"
                style={{ fontSize: "2.25rem", color: "var(--accent-primary)" }}
              >
                {avgWeeklyLoss}
                <span className="stat-unit">kg</span>
              </p>
              ...
              >
                Média de perda por semana
              </p>
            </div>

            <div
              className={`glass-panel ${!isCapturing ? "anim-enter anim-delay-3" : ""}`}
              style={{ padding: "1.5rem", opacity: 1, transform: "none" }}
            >
            ...
            <div
              className={`glass-panel ${!isCapturing ? "anim-enter anim-delay-4" : ""}`}
              style={{ padding: "1.5rem", opacity: 1, transform: "none" }}
            >
              <p className="stat-label">Dose Atual</p>
              <p className="stat-number" style={{ fontSize: "2.25rem" }}>
                {currentDose > 0 ? currentDose.toFixed(1) : "—"}
